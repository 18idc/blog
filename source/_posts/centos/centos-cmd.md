...
## ssh免密登录
* 在需要免密的电脑上生成密钥和公钥 ssh-keygen -t rsa -C "hi@q18idc.com"
* 把公钥中的内容追加到服务器~/.ssh/authorized_keys文件的末尾[没有就创建]
* 关闭秘密登录功能，编辑vim /etc/ssh/sshd_config文件，把PasswordAuthentication no设置no
...
source /etc/profile
```
* 检查是否安装成功，老办法打印一下JAVA的版本信息
```bash
java -version
```

### 2. 方法二
* 查看可以安装的jdk版本yum list java*
* 安装1.8的JDKyum install java-1.8.0-openjdk.i686
* 查看jdk是否安装成功java -version
...
* 把8080端口修改为80端口
```bash
vi /usr/local/apache-tomcat-9.0.0.M18/conf/server.xml
```
* 配置管理帐户
```bash
vi /usr/local/apache-tomcat-9.0.0.M18/conf/tomcat-users.xml

<role rolename="manager"/>     
 <role rolename="admin"/> 
 <role rolename="admin-gui"/>
 <role rolename="manager-gui"/>
 <user username="q18idc" password="q18idc.com" roles="admin-gui,manager-gui"/>
```
* 解决403错误
```bash
#修改context.xml文件  把节点注释掉
vi /usr/local/apache-tomcat-9.0.0.M18/webapps/manager/META-INF/context.xml
```
